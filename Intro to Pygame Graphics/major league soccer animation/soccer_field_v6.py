...
ck = (127, 33, 33)
day_rain = (121, 155, 196)
night_rain = (50, 79, 114)
...
RAINY = pygame.Surface((800, 600))
RAINY.set_alpha(150)
RAINY.fill((124, 118, 135))
...
def draw_rain(x, y):
    pygame.draw.ellipse(RAINY, rain_color, [x, y, 5, 5])
...
while not done:
    ...
    for rc in rain_clouds:
        rc[0] -= 0.5

        if rc[0] < -100:
            rc[0] = random.randrange(800, 1600)
            rc[1] = random.randrange(0, 150)

            
    for a in rain:
        a[1] += random.randint(2, 4)
        a[0] -= random.randint(2, 3)

        if a[1] > 800:
            a[0] = random.randrange(0, 1000)
            a[1] = random.randrange(-200, 800)
